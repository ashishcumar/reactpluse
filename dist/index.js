  ...
  React.useState = function useStatePatched(initialState) {
    ...
    if (currentComponent) {
      ...
      const wrappedSetter = function(newState) {
        _chunkACJSZERPjs.recordStateUpdate.call(void 0, currentComponent);
...
function openDebugPanel(componentName, renderCount, stack, componentPath, allCounts, relationships, stateUpdates) {
  ...
  const stateUpdatesHtml = stateUpdates ? `<div class="state-updates"><strong>State updates by component:</strong><br>${Object.entries(stateUpdates).sort((a, b) => b[1] - a[1]).map(([name, updates]) => {
    const renders = _optionalChain([allCounts, 'optionalAccess', _ => _[name]]) || 0;
...
function enableReactPulse(options) {
  if (typeof process !== "undefined" && _optionalChain([process, 'access', _2 => _2.env, 'optionalAccess', _3 => _3.NODE_ENV]) === "production") {
    return;
  }
  if (isEnabled) return;
  console.log("[ReactPulse] Initializing\u2026");
  _chunkACJSZERPjs.setOnLoopDetected.call(void 0, (componentName, count, stack, componentPath, allCounts, relationships, stateUpdates) => {
    console.warn(
  ...
  patchCreateElement(options.React, _chunkACJSZERPjs.recordRender);
  try {
...
exports.enableReactPulse = enableReactPulse; exports.recordRender = _chunkACJSZERPjs.recordRender;
